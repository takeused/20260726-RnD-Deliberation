...
- [x] ① 사후 검증 루프: 예상 질의 JSON 저장(07b) + `--verify-questions`로 실제 질의 대조·적중률 기록
- [x] ② 토론층 교체: Pro/Con → 심사위원 패널(공격) vs 사업 발표자(방어)
- [x] ③ 모의 청문 모드: `--hearing`으로 예상 질의 대화형 리허설 + 답변 평가·압박 후속질의
- [x] ④ 관문 프로파일(--gate: 부처심의/예타/예산조정/국회) + 질의 은행(전형 패턴 시드, 검증된 실제 질의 자동 축적)
