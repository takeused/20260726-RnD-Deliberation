...
- 저장 구조: 프로젝트 내부 `results/{project_id}/{execution_id}/` 아래 번호 접두사 md 파일. 과정 기록이 본체이므로 전부 저장.
...
## 7단계 설계 결정 (원천 목표 정렬 개조)

- 전면 재설계 대신 점진 개조. 하부(입력·산출물·메모리·테스트)는 유지, 중간층(토론)과 상부(측정)만 교체.
- git 저장소 초기화 (af1ae00 베이스라인). 대규모 구조 변경 전 롤백 수단 확보 목적.
- ① 적중률 판정은 LLM 의미 매칭(적중/부분적중/미적중), 적중률 계산은 코드에서 수행(LLM 숫자 불신 원칙 유지).
  이력은 jsonl로 누적해 회차별 적중률 추세를 볼 수 있게 한다.
- ② Pro Reviewer 제거 이유: 실제 심의장에 옹호 위원은 없음. 옹호자는 발표자(사용자) 본인.
  ReviewDebateState 필드를 defender/examiner로 개명 — 상태 의미와 코드가 어긋나는 것 방지.
- ③ 모의 청문은 LangGraph가 아닌 단순 대화 루프. 인터랙티브 stdin 기반이라 그래프 오케스트레이션 불필요.
- ④ 질의 은행 시드는 "전형 질의 패턴"으로 명확히 라벨링 (실제 기출이라고 주장하지 않음).
  ①의 검증을 거친 실제 질의가 은행에 자동 축적되어 시간이 지날수록 실제 기출 비중이 커지는 구조.
